...
class Compilation extends Tapable {
	...
	seal(callback) {
		...
		this.applyPluginsAsyncSeries("optimize-tree", this.chunks, this.modules, (err) => {
			...
			if(this.applyPluginsBailResult("should-generate-chunk-asset") !== false) {
			...
			this.applyPluginsAsync("additional-asset", err => {
				if(err) {
					return callback(err);
				}
				this.applyPluginsAsync("optimize-chunk-asset", this.chunks, err => {
					if(err) {
						return callback(err);
					}
					this.applyPlugins1("after-optimize-chunk-asset", this.chunks);
					this.applyPluginsAsync("optimize-asset", this.assets, err => {
						if(err) {
							return callback(err);
						}
						this.applyPlugins1("after-optimize-asset", this.assets);
